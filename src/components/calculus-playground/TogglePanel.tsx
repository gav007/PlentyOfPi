...
interface TogglePanelProps {
  ...
  onShowDerivativeChange: (checked: boolean) => void;
  showTangent: boolean;
    ...
    <Card className="bg-muted/20">
      ...
      <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
        <div className="flex items-center justify-between sm:justify-start space-x-2 p-3 bg-background/50 rounded-lg shadow-sm">
          <Label htmlFor="show-derivative-toggle" className="flex items-center gap-1.5 text-sm font-medium">
            <TrendingUp className="w-4 h-4 text-chart-2"/>
            Derivative f'(x)
          </Label>
          <Switch
            id="show-derivative-toggle"
            checked={showDerivative}
            onCheckedChange={onShowDerivativeChange}
            aria-label="Toggle derivative line visibility"
          />
        </div>
        <div className="flex items-center justify-between sm:justify-start space-x-2 p-3 bg-background/50 rounded-lg shadow-sm">
