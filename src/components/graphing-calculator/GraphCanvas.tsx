...
const formatTick = (tick: any) => (typeof tick === 'number' ? tick.toFixed(1) : tick);
...
export default function GraphCanvas({
  ...
  const [tempRange, setTempRange] = React.useState({
    yMin: String(range.yMin), // Store as string for input field
  ...
  const handleApplyDomain = () => {
    if (tempDomain.xMin < tempDomain.xMax) {
      onDomainChange(tempDomain);
    }
  };
  ...
  const visiblePlotData = plotData.filter(pd => {
    ...
    return expr ? expr.visible : false;
  });
      ...
      <Card className="bg-muted/20 p-4 shadow-inner">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
          <div>
            ...
            <Input id="xMin" type="number" value={tempDomain.xMin} onChange={e => setTempDomain(d => ({ ...d, xMin: parseFloat(e.target.value) || 0 }))} className="h-8 text-sm" />
          </div>
          <div>
            <Label htmlFor="xMax" className="text-xs">X Max</Label>
            <Input id="xMax" type="number" value={tempDomain.xMax} onChange={e => setTempDomain(d => ({ ...d, xMax: parseFloat(e.target.value) || 0 }))} className="h-8 text-sm" />
          </div>
          <div>
            <Label htmlFor="yMin" className="text-xs">Y Min</Label>
            <Input id="yMin" type="text" value={tempRange.yMin} onChange={e => setTempRange(r => ({ ...r, yMin: e.target.value }))} placeholder="auto" className="h-8 text-sm" />
          </div>
          <div>
            <Label htmlFor="yMax" className="text-xs">Y Max</Label>
            <Input id="yMax" type="text" value={tempRange.yMax} onChange={e => setTempRange(r => ({ ...r, yMax: e.target.value }))} placeholder="auto" className="h-8 text-sm" />
          </div>
      ...
      <div className="w-full aspect-[1.5/1] min-h-[350px] md:min-h-[450px] bg-background border rounded-md shadow-lg p-2 pr-4">
        {visiblePlotData.length > 0 ? (
            ...
            <LineChart margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
              ...
              {visiblePlotData.map(plot => (
                <Line
                  ...
                  isAnimationActive={false} // Better performance for dynamic updates
                  connectNulls={true} // Handle discontinuities better
                />
              ))}
            </LineChart>
          </ResponsiveContainer>
          ...
          <div className="flex items-center justify-center h-full text-muted-foreground">
            Enter an expression to see it plotted.
          </div>
        )}
      </div>
    </div>
