...
import type { QuizItem, QuizQuestion } from '@/types/lessons';
import { useState, useEffect } from 'react';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import Link from 'next/link';
import { ArrowLeft, ArrowRight, HelpCircle, CheckCircle, XCircle, RotateCcw } from 'lucide-react';
...
export default function QuizPageContent({ quiz, moduleTitle, prevLink, nextLink }: QuizPageContentProps) {
  ...
  const handleSubmit = () => {
    ...
    quiz.questions.forEach((q, index) => {
      ...
      feedbackVisibility[index] = true; // Show feedback for all questions after submission
    });
    ...
    <Card className="shadow-xl w-full">
      <CardHeader>
        ...
        <CardTitle className="text-2xl md:text-3xl font-bold flex items-center gap-2">
          <HelpCircle className="w-7 h-7" /> Quiz: {quiz.title}
        </CardTitle>
      </CardHeader>
              ...
              {Object.entries(q.options).map(([key, text]) => (
                ...
                >
                  ...
                  <Label 
                    ...
                    className={cn(
                      "font-normal text-sm flex-1 py-1", // Added py-1 for vertical padding within label
