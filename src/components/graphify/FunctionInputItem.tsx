...
export default function FunctionInputItem({ func, onUpdate, onDelete, isOnlyFunction }: FunctionInputItemProps) {
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <div className="flex flex-col gap-1 py-1.5 border-b border-border last:border-b-0 hover:bg-muted/30 transition-colors group">
          ...
          {func.error && (
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="absolute right-16 top-1/2 -translate-y-1/2 h-7 w-7 flex items-center justify-center transform translate-y-[-18px] sm:translate-y-[-50%]"> {/* Adjust vertical position based on context */}
                   <AlertTriangle className="w-4 h-4 text-destructive" />
                </div>
              </TooltipTrigger>
              <TooltipContent side="top" className="bg-destructive text-destructive-foreground text-xs max-w-xs p-2">
                <p id={`func-error-${func.id}-tooltip`}>{func.error}</p>
              </TooltipContent>
            </Tooltip>
          )}
          <Button
        ...
        {func.error && (
          <p id={`func-error-${func.id}`} className="text-xs text-destructive px-2 ml-8 sm:ml-10 text-left break-words">
             {func.error}
          </p>
        )}
      </div>
